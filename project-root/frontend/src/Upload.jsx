...
import './Upload.css'; // All styles
import { FaFilePdf, FaTimes, FaUpload, FaPlay, FaSpinner, FaPaperclip, FaChevronLeft, FaChevronRight } from "react-icons/fa";
...
function Upload() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const sortedTasks = data.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); // Sort by creation date
  ...
  const handleScrollClick = (direction) => {
    if (taskListRef.current) {
      const scrollAmount = 380; // Approximate width of a task card + gap
      if (direction === 'left') {
        taskListRef.current.scrollLeft -= scrollAmount;
      } else {
        taskListRef.current.scrollLeft += scrollAmount;
      }
    }
